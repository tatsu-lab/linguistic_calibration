...
from accelerate.utils import wait_for_everyone, gather
...
def score_sequences_with_accelerate_given_model(
    model: nn.Module,
    ...
    state = PartialState()
    local_rank, world_size = state.process_index, state.num_processes
    ...
    return_rewards = []
    with state.split_between_processes(new_sequences, apply_padding=False) as per_device_sequences:
        for batch_idx, start_idx in tqdm.tqdm(
            enumerate(range(0, len(per_device_sequences), per_device_batch_size)),
            desc="evaluating rewards for batches",
            total=len(per_device_sequences) // per_device_batch_size,
            disable=not state.is_main_process,
        ):
            batch = per_device_sequences[start_idx : start_idx + per_device_batch_size]
            source = tokenizer(
                batch,
                return_tensors="pt",
                padding="max_length",
            ...
            source = common.prepare_inputs(source, device=state.device)

            if verbose:
                logger.info(f"RANK {local_rank} waiting for everyone before reward scoring, idx = {batch_idx}", main_process_only=False)
            wait_for_everyone()
            if verbose:
                logger.info(f"RANK {local_rank} finished waiting for everyone before reward scoring, idx = {batch_idx}", main_process_only=False)

            if verbose:
                logger.info(f"RANK {local_rank} starting reward scoring, idx = {batch_idx}", main_process_only=False)
            rewards = model(input_ids=source.input_ids, attention_mask=source.attention_mask).rewards
            if verbose:
                logger.info(f"RANK {local_rank} finished reward scoring, idx = {batch_idx}", main_process_only=False)

            return_rewards.append(rewards)

    logger.info(f"RANK {local_rank} waiting for everyone before emptying cache")
    wait_for_everyone()
    logger.info(f"RANK {local_rank} finished waiting for everyone before emptying cache")

    logger.info(f"RANK {local_rank} emptying cache")
    torch.cuda.empty_cache()
    return_rewards = torch.cat(return_rewards, dim=0)
    logger.info(f"RANK {local_rank} finished emptying cache")

    # Block until all processes have reached this point.
    logger.info(f"RANK {local_rank} waiting for everyone before all_gather")
    wait_for_everyone()
    logger.info(f"RANK {local_rank} finished waiting for everyone before all_gather")

    logger.info(f"RANK {local_rank} starting all_gather. Shape: {return_rewards.shape}")
    return_rewards = gather(return_rewards)
    logger.info(f"RANK {local_rank} finished all_gather. Shape: {return_rewards.shape}")
    return return_rewards.tolist()[:ori_data_size]
