...
from linguistic_calibration.inference.llama_2_chat_decode_utils import llama_2_chat_format_prompts
from .. import common, constants, distributed_utils, logging, torch_ops, utils, openai_utils, anthropic_utils
...
def respond_to_prompts_with_huggingface_given_wrapped_model(
    model,
    tokenizer: transformers.PreTrainedTokenizer,
    prompts: Sequence[str],
    is_policy: bool,
    temperature: float = None,
    decoding_args: HFDecodingArguments = None,
    ...
    **decoding_kwargs,
    ...
    """Decode from a given FSDP-wrapped model a sequence of string prompts."""
    local_rank, world_size = distributed_utils.setup()
    device = torch.device("cuda", local_rank) if torch.cuda.is_available() else torch.device("cpu")
    ...
    if decoding_args is not None:
        ...
        generate_kwargs.update(decoding_kwargs)
    ...
    for batch_idx, start_idx in tqdm.tqdm(
        ...
        if is_policy:
            ...
            sequences = model.respond(
                queries=inputs,
                query_attn_masks=attention_mask,
                temperature=temperature
            )['responses']
        else:
            assert generate_kwargs is not None
            sequences = model.generate(inputs=inputs, attention_mask=attention_mask, **generate_kwargs)
            sequences = torch_ops.right_pad(
                sequences[:, inputs.size(1):],
                target_size=(sequences.size(0), decoding_args.max_new_tokens),
    ...
    if world_size > 1 and divide_work:
        ...
        mine = einops.rearrange(completions, "(n d) l -> n d l", d=1)
...
def decode_prompts_with_huggingface_given_model(
    ...
    max_instances=sys.maxsize,
    pad_to_length=4096,  # Force pad to this length for distributed communication to work.
    min_new_tokens=300,  # If the prefix is too long, we can adjust max_new_tokens. However, this is a lower bound.
    tf32=True,
    ...
    **decoding_kwargs,
    ...
    # Log the generate_kwargs
    logger.warning(f"generate_kwargs = {generate_kwargs}")
    ...
    for batch_idx, start_idx in tqdm.tqdm(
        ...
        if any(llama_chat_model_name in model.base_model.config._name_or_path
               for llama_chat_model_name in constants.LLAMA_2_CHAT_MODELS):
            input_ids = llama_2_chat_format_prompts(batch, tokenizer)
        ...
        if inputs.shape[1] + decoding_args.max_new_tokens > pad_to_length:
            # Set max_new_tokens to the maximum possible value to avoid errors
            adjusted_max_new_tokens = pad_to_length - inputs.shape[1]
            if adjusted_max_new_tokens < min_new_tokens:
                raise ValueError(
                    f"Prefix is too long. Cannot adjust max_new_tokens to be at least {min_new_tokens}. "
                    f"Please reduce the length of the prefix.")
            generate_kwargs["max_new_tokens"] = adjusted_max_new_tokens
            logger.warning(
                f"Setting max_new_tokens to {adjusted_max_new_tokens} to avoid out of bound errors for batch index "
                f"{batch_idx}. This may cause incomplete generations."
            )
        else:
...
def decode_prompts_with_model(
    ...
    **kwargs
    ...
    if model_name in constants.OPENAI_MODELS:
        return openai_utils.openai_completion(
            prompts=prompts,
            decoding_args=decoding_args,
            model_name=model_name,
            **kwargs,
        )
    elif model_name in constants.ANTHROPIC_MODELS:
        return anthropic_utils.anthropic_completion(
            prompts=prompts,
            decoding_args=decoding_args,
            model_name=model_name,
            **kwargs,
        ...
        return decode_prompts_with_huggingface(
...
def get_text_from_completions(completions):
    # Strip newlines at beginning and end
    to_return = []
    for completion in completions:
        if hasattr(completion, 'text'):
            to_return.append(completion.text.strip())
        elif isinstance(completion, float):
            to_return.append(completion)
        elif isinstance(completion, str):
            to_return.append(completion.strip())
        elif isinstance(completion, list):
            to_return.append(get_text_from_completions(completion))

    return to_return


def get_decoding_args(model_name):
    if model_name in constants.OPENAI_MODELS or model_name in constants.ANTHROPIC_MODELS:
        is_chat_model = openai_utils.requires_chatml(model_name)
        if is_chat_model:
            decoding_args = openai_utils.OpenAIDecodingArgumentsChat()
        else:
            decoding_args = openai_utils.OpenAIDecodingArguments()
    else:
        decoding_args = HFDecodingArguments(max_new_tokens=512)
