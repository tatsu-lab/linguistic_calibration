...
from typing import Any, Callable, Dict, Mapping, Optional, Sequence, Union, Tuple
...
def get_pretrained_model_name_with_model_name_or_path(model_name_or_path: AnyPathOrNone) -> Optional[str]:
    ...
    # When recursive finding doesn't work, we revert to the dumb hardcoded rule.
...
def get_model_family_with_model_name_or_path(model_name_or_path: AnyPath) -> str:
    pretrained_model_name = get_pretrained_model_name_with_model_name_or_path(model_name_or_path)
    try:
        return constants.MODEL_NAME_TO_FAMILY[pretrained_model_name]
    except KeyError:
        raise ValueError(
            f"{model_name_or_path} is not in the set of admissible model families. "
            f"Expected model families: {list(constants.MODEL_NAME_TO_FAMILY.values())}. "
        )
...
def make_generative_lm(
    ...
    **kwargs,
    ...
    model_family = get_model_family_with_model_name_or_path(model_name_or_path)
    ...
    if flash_attn:
        ...
        logger.warning("Attempting to use Flash Attention.", main_process_only=True)
    else:
        logger.warning("Using standard model (no Flash Attention).", main_process_only=True)

    if model_family == 'llama':
        model_cls = transformers.LlamaForCausalLM
    elif model_family in {'redpajama'}:
...
def get_model_path_or_paths_from_short_name(
    model_name
) -> Union[str, Tuple[str]]:
    try:
        model_path = constants.SHORT_NAME_TO_MODEL_PATH[model_name]
    except KeyError:
        logger.info(f"Model name {model_name} not found in "
                     f"constants.SHORT_NAME_TO_MODEL_PATH. "
                     f"Assuming it is a path to a model.")
...
def assert_models_available(
    models_for_run: Union[str, Sequence[str]]
):
    if isinstance(models_for_run, str):
    ...
    for model_name in models_for_run:
        ...
        if model_name in constants.SHORT_NAME_TO_MODEL_PATH.keys():
            continue

        raise ValueError(
            f"Model {model_name} is not available. "
            f"Available models: {constants.SHORT_NAME_TO_MODEL_PATH.keys()}. "
            f"Available OpenAI models: {constants.OPENAI_MODELS}."
            f"Available Anthropic models: {constants.ANTHROPIC_MODELS}."
        )
