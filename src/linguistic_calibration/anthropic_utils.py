...
from linguistic_calibration.openai_utils import (
    OpenAIDecodingArguments, convert_dict_to_openai_object, prompt_to_chatml, requires_chatml)
...
def convert_args_oai_to_anthropic_text_completion(decoding_args: dict):
    args_mapping = {
...
def convert_args_oai_to_anthropic_chat_completion(decoding_args: dict):
    args_mapping = {
        "stop": "stop_sequences",
    }
    for arg in args_mapping:
...
def _anthropic_completion_helper(
    ...
    if is_chat:
        shared_kwargs = convert_args_oai_to_anthropic_chat_completion(shared_kwargs)
    else:
        shared_kwargs = convert_args_oai_to_anthropic_text_completion(shared_kwargs)
