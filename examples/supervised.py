...
from linguistic_calibration import common, constants, data_utils, logging, utils
...
class DataArguments:
    dataset_path: str = field(
        ...
        metadata={
            ...
            "The default HF path contains intermediate results for SFT on summaries or answer extractions."
        },
    )
    eval_size: int = field(
        default=500,
    ...
    prompt_template_path: str = field(
        default=(pathlib.Path(__file__).parent.parent / "src" / "linguistic_calibration" / "prompts" / "train" /
                 "extract_answers_llama_finetuned.txt"),
    ...
    dataset_name: str = field(
        ...
        metadata={"help": "Name of the dataset to load -- the argument `name` passed to `datasets.load_dataset`."
                          "Will be set according to the SFT type."}
    )
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    sft_type: Literal["lc_sft", "extract_answers", "factuality_sft", "claude_distill"] = field(
        default="extract_answers",
        metadata={"help": "Type of the SFT model. By default, uses the ExtractAnswers supervision. This determines "
...
def main():
    ...
    if any(model_class in model_args.model_name_or_path.lower()
           for model_class in constants.GPTNEOX_MODEL_CLASSES):
    ...
    data_module: dict = data_utils.make_linguistic_calibration_supervised_data_module(
        tokenizer=tokenizer,
        data_args=data_args,
        training_args=training_args,
    )

    # TODO(@nband): should be able to remove if your Llama 2 7B model has recently been pulled from HF Hub.
    #   Previous versions did not have a `do_sample` attribute.
